...
similarWordUsage <- function (entity1 = file.choose(), entity2 = file.choose()) {  
  library(dplyr)
  library(sqldf)
  ...
  #Uses SQL to produce table z, which produces a table with each word in both datasets and with
  # a confidence Level above 0 with word, frequency, confidence intervals, porportion, etc. 
  z <-sqldf("SELECT DISTINCT * FROM data1, data2 WHERE data1.Word = data2.Word AND data1.ConfidenceLow > 0")
  
  #Ini
  same <- data.frame(matrix(ncol = length(z)), stringsAsFactors = FALSE)
  
  #Produces similar frequency of word usage from words in both datasets
  for (i in 1:length(z[,2])) {
    if (z[i,6] >= z[i,13] && z[i,6] <= z[i,14]) {
      same[i,] <- z[i,]     
    } else if (z[i,7] >= z[i,13] && z[i,7] <= z[i,14]) {
      same[i,] <- z[i,]
    }
    
  }
  
  #Removes NAs from same
  same <- same[complete.cases(same),1:length(z)]

  #Orders different by greatest difference in confidence interval
  same <- sqldf("SELECT X1 AS D1Word, (X6 - X14)*1000 AS differenceStrength, X2 AS D1Frequency, X4 AS D1Proportion, X5 AS D1MarginOfError, X6 AS D1ConfidenceLow, X7 AS D1ConfidenceHigh, X9 AS D2Frequency, X11 AS D2Proportion, X12 AS D2MarginOfError, X13 AS D2ConfidenceLow, X14 AS D2ConfidenceHigh   FROM same ORDER BY (X6 - X14)")
  return(same)
}
